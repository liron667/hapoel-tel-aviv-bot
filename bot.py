...
def collect_wp(sources, cutoff):
    """WordPress REST posts (…/wp-json/wp/v2/posts) for JS-rendered sites."""
    items = []
    for src in sources or []:
        try:
            data = requests.get(src["url"], headers={"User-Agent": UA}, timeout=20).json()
            for p in data if isinstance(data, list) else []:
                when = None
                dt = p.get("date_gmt")
                if dt:
                    try:
                        when = datetime.fromisoformat(dt).replace(tzinfo=timezone.utc)
                    except ValueError:
                        when = None
                if not within_lookback(when, cutoff):
                    continue
                title = html.unescape(
                    re.sub("<[^>]+>", " ", (p.get("title") or {}).get("rendered", "")).strip()
                )
                link = p.get("link", "")
                items.append({
                    "id": link or str(p.get("id")), "title": title, "summary": "",
                    "link": link, "source": src["name"],
                })
        except Exception as ex:
            print(f"[warn] WP {src.get('name')} failed: {ex}")
    return items
...
def main():
    ...
    items = (collect_rss(cfg.get("rss_feeds"), cutoff)
             ...
             + collect_wp(cfg.get("wp_sources"), cutoff)
